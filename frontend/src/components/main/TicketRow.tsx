...
const TicketRow = ({ticket}:{ticket:ticket}) => {
  ...
  newSearchParams.set('search', ticket.ref);
    ...
    const suffPAth=ticket.creator?._id===user?._id?"/tickets/sent":"/tickets"
  ...
  return (
    <Link className="flex flex-col items-start rounded-lg bg-gray-hot/20 mx-2 my-1 p-2 gap-2 " to={{
    pathname: `${suffPAth + status}`,
    search: `?${newSearchParams.toString()}`
  }} >
      <div className="flex justify-between px-2 w-full ">
        <p className="italic capitalize text-[10px] font-semibold">{ticket.ref}</p>
        <p className="italic capitalize text-[10px] rounded-full px-1 py-px" style={{backgroundColor:color}} >{ticket.status} </p>
      </div>
      <p className=" capitalize text-xs">{ticket.motif.split("--------")[1]} </p>
      <p className=" capitalize text-xs"><span className="font-semibold">Créer Par :</span> {ticket.creator?.name} </p>
          <p className=" capitalize text-xs"><span className="font-semibold">Organisation :</span> {ticket.emitterOrganization?.name} </p>
